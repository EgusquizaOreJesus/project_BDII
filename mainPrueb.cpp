...
int main() {
    const char* input =
            "CREATE TABLE Youtube FROM FILE 'YTStats2.csv' USING INDEX AVL('anime_id');";
...
//            "SELECT * FROM Youtube WHERE key = 'e-wJyFk9wls';";
//            "SELECT * FROM Youtube WHERE key BETWEEN 'e04pTwZdLhg' AND 'e0XDEBPDirs';";
...
//    Scanner scanner(input);
//    Parser<const char*> parser2(&scanner, parser.getStructure());         // para cargar data de apps
