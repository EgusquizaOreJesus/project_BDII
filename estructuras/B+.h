...
string indexfile = "indexYoutubeBplus.bin";
string datafile = "dataYoutubeBplus.bin";
...
public:
    ...
    void insert(Record<Tk> record )
    {
        ...
        int l_default = -1;
        Tk key_default;
        cout<<"insertando"<<record.key<<endl;
        bool flag = false;
...
//                cout<<"SE INSERTO EL EXCESO ACA"<<endl;
    ...
    Record<Tk> search(const char* key)
    {
        ...
        Record<Tk>* ans_ptr= search(key , 0 , file_index , file_data);
        if(ans_ptr == nullptr) return Record<Tk>();

        Record<Tk> ans = *ans_ptr;
        file_index.close();
        file_data.close();
        return ans;
    }

    Record<Tk> * search(const char * key , int current_node  , fstream & file_index , fstream & file_data)
    {
        indexNode<Tk> node = read_index_node_in_pos(file_index , current_node);


        if(node.is_last and node.count == 0)
        {



            dataPage<Tk> search_page = read_page_node_in_pos(file_data , 0);
    ...
    vector<Record<Tk>> range_search(const char * beg , const char * fin ){
        fstream file_index(indexfile , ios::binary | ios::in|ios::out);
        fstream file_data(datafile , ios::binary |ios::in | ios::out);
    ...
    vector<Record<Tk>> range_search  (fstream & file_index , fstream & file_data,const char * beg , const char * fin , int current_pos ) {
        ...
        if(node.is_last and node.count==0)
        {
            ...
            return linked_sweep(file_data , beg , fin , 0);
        }
...
private:
    ...
    vector<Record<Tk>> linked_sweep(fstream & file,  const char * beg , const char * end , int pos    ){
    ...
    Record<Tk>* search_in_page(dataPage<Tk>& page, const char * key)
    {
        int left = 0;
